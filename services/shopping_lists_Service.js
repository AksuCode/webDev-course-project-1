...
const listCount = async (name) => {
    return (await sql`SELECT COUNT(*) FROM shopping_lists WHERE name = ${name}`)[0].count;
}
...
export { createShoppinglist, findAllActiveShoppinglists, listCount, listMarkListActiveAsFalse }